...
import colors from "../nix/colors.js";
...
function validateConfig() {
    ...
    if (!config.OWNER || config.OWNER.trim() === "") {
        console.log(`${colors.fg.red}[ERROR] OWNER name is missing in config/config.main.json${colors.reset}`);
        console.log(`${colors.fg.red}[ERROR] Please add OWNER name to continue${colors.reset}`);
        console.log(`${colors.fg.red}[ERROR] XaviaBot has stopped.${colors.reset}`);
        process.exit(1);
    }
}
...
function displayBotInfo() {
    const config = global.config;
    console.log();
    console.log(`${colors.fg.cyan}BOT INFO${colors.reset}`);
    console.log(`${colors.fg.green}BOT NAME:${colors.reset} ${colors.fg.white}${config.NAME || 'Xavia'}${colors.reset}`);
    console.log(`${colors.fg.green}PREFIX:${colors.reset} ${colors.fg.white}${config.PREFIX || 'x'}${colors.reset}`);
    console.log(`${colors.fg.green}VIP UID:${colors.reset} ${colors.fg.white}${config.VIP?.length > 0 ? config.VIP.join(', ') : 'None'}${colors.reset}`);
    console.log(`${colors.fg.green}ADMIN UID:${colors.reset} ${colors.fg.white}${config.MODERATORS?.join(', ') || 'None'}${colors.reset}`);
    console.log(`${colors.fg.green}BOT UID:${colors.reset} ${colors.fg.white}${global.botID || 'Loading...'}${colors.reset}`);
    console.log();
    console.log(`${colors.fg.magenta}OWNER INFO${colors.reset}`);
    console.log(`${colors.fg.yellow}OWNER:${colors.reset} ${colors.fg.white}${config.OWNER}${colors.reset}`);
    console.log(`${colors.fg.yellow}NOTE:${colors.reset} ${colors.fg.white}${config.OWNER_NOTE || 'enjoy Xavia bot'}${colors.reset}`);
    console.log();
}
...
/**
 * @param {import("fca-aryan").IFCAU_API} api
