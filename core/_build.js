...
import startAppstateRefresh, { stopAppstateRefresh } from "./nix/refresh.js";
    ...
    return new Promise((resolve, reject) => {
        ...
        loginState()
            .then(async (api) => {
                ...
                refreshState();
                startAppstateRefresh(); // Start 20-minute appstate refresh
...
function loginState() {
    const { APPSTATE_PATH, APPSTATE_PROTECTION, LOGIN } = global.config;

    return new Promise((resolve, reject) => {
        if (LOGIN === true) {
            const loginConfigPath = resolvePath(process.cwd(), "config", "login.main.json");
            if (!global.isExists(loginConfigPath, "file")) {
                return reject(new Error("login.main.json not found. Please create it with email and password."));
            }

            const loginConfig = JSON.parse(readFileSync(loginConfigPath, "utf8"));
            if (!loginConfig.email || !loginConfig.password) {
                return reject(new Error("Email or password not found in login.main.json"));
            }

            const options = global.config.FCA_OPTIONS;
            login({ email: loginConfig.email, password: loginConfig.password }, options, (error, api) => {
