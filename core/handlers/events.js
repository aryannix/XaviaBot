...
async function handleReaction(event) {
    ...
    const unsendEmojis = global.config?.unsend || [];
    const isAdmin = global.config?.MODERATORS?.includes(userID) || global.config?.ABSOLUTES?.includes(userID);
    
    if (unsendEmojis.includes(reaction) && isAdmin) {
        try {
