...
function isVipUser(userID) {
    const vipList = global.config.VIP || [];
...
async function handleCommand(event, xDatabase) {
    ...
    if (args.length > 0 && args[0].startsWith(prefix)) {
        ...
        if (potentialCommand) {
            ...
            if (commandConfig?.nixprefix === true) {
    ...
    if (commandName) {
        ...
        if (command !== null && commandInfo) {
            if (commandInfo.vip === true && !isVipUser(senderID)) {
                return api.sendMessage("𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐚 𝐕𝐈𝐏 𝐮𝐬𝐞𝐫", threadID, messageID);
            }
...
function isAdmin(userID) {
    const moderators = global.config.MODERATORS || [];
    const absolutes = global.config.ABSOLUTES || [];
    return moderators.includes(userID) || absolutes.includes(userID);
}

async function handleUnsendReaction(event) {
    const { api } = global;
    const unsendConfig = global.config.UNSEND;
    
    if (!unsendConfig?.enabled) return false;
    
    const unsendEmoji = unsendConfig.emoji || "😠";
    const { messageID, userID, reaction } = event;
    
    if (reaction === unsendEmoji && isAdmin(userID)) {
        try {
...
async function handleReaction(event, xDatabase) {
    ...
    const unsendHandled = await handleUnsendReaction(event);
    if (unsendHandled) return;
