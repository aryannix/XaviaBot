...
async function onCall({ message, args, getLang }) {
    ...
    const path = `${global.cachePath}/drake_${message.senderID}_${Date.now()}.png`;
    const clean = () => {
        try {
    ...
    global
        .downloadFile(path, `${global.xva_api.popcat}/drake?text1=${encodeURIComponent(text1)}&text2=${encodeURIComponent(text2)}`)
        .then(async _ => {
